...
var api = module.exports = {
    ...
    */
    getFlows: function(opts) {
        return new Promise(function(resolve,reject) {
            ...
            // Clone active config first to not screw up things of other users!
            var activeConfig = clone(
                runtime.nodes.getFlows()
            );

            // Return to the current user only his own flows during request
            if (opts && opts.user) {
                activeConfig.flows = activeConfig.flows.filter(function(flow) {
                    if (flow.user) {
                        return flow.user.username && flow.user.username == opts.user.username;
                    }
                    return false;
                });
            }

            return resolve(activeConfig);
        });
    },
    /**
    ...
    */
    setFlows: function(opts) {
        return new Promise(function(resolve,reject) {
            ...
            if (deploymentType === 'reload') {
                ...
                api.assignUserToFlows(flows.flows, user);
                flows.flows = api.mergeUserFlowsIntoCommonConfig(flows.flows, user);

                apiPromise = runtime.nodes.setFlows(flows.flows,deploymentType,null,null,user);
            }
    ...
    assignUserToFlows: function(currUserFlows, user) {
        if (currUserFlows && currUserFlows.length) {
            currUserFlows.forEach(function(flow) {
                flow.user = user;
            });
        }
    },

    mergeUserFlowsIntoCommonConfig: function(currUserFlows, user) {
        var activeConfig = runtime.nodes.getFlows();
        var allFlows = activeConfig.flows;

        var otherUsersFlows = allFlows.filter(function(flow) {
            if (flow.user) {
                return flow.user.username && flow.user.username !== user.username;
            }

            return false;
        });

        return currUserFlows.concat(otherUsersFlows);
    },
